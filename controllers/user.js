...
module.exports.signup = async (req, res, next) => {
  try {
    ...
    req.login(newRegister, async (err) => {
      ...
      req.flash(
        "success",
        `Hey! ${req.user.name}, your account was created successfully.`,
      );
      res.redirect("/home");
    });
  } catch (e) {
...
module.exports.requestPasswordReset = async (req, res) => {
  try {
    ...
    if (tracker && tracker.attempts >= 2) {
      return res
        .status(429)
        .json({ success: false, message: "Limit reached. Try tomorrow." });
    }
...
module.exports.randomUserProfile = async (req, res) => {
  try {
    ...
    const isSynced = req.user.synced.some((syncedId) =>
      syncedId.equals(profileUser._id),
    );
...
module.exports.syncRequestsRender = async (req, res) => {
  ...
  let receivedRequests = await Connection.find({
    receiver: userId,
    status: "pending",
  }).populate("sender");

  // 2. Fetch Sent Requests (Outgoing)
  let sentRequests = await Connection.find({
    sender: userId,
    status: "pending",
  }).populate("receiver");
...
module.exports.acceptSync = async (req, res) => {
  ...
  let connection = await Connection.findByIdAndUpdate(connectionId, {
    status: "accepted",
  });
...
module.exports.deleteAccount = async (req, res) => {
  try {
      ...
      for (let post of user.post) {
        if (post.image && post.image.fileId) {
          try {
            ...
            console.log(
              `Post image delete failed for ${post._id}: ${err.message}`,
            );
          }
        }
      }
    ...
    if (
      ...
      user.profilePic.fileId !== "default"
    ) {
      try {
    ...
    if (
      user.coverPic &&
      user.coverPic.fileId &&
      user.coverPic.fileId !== "default"
    ) {
      try {
...
module.exports.updatePicture = async (req, res) => {
  try {
    ...
    const oldFileId = userData[targetField]
      ? userData[targetField].fileId
      : null;
